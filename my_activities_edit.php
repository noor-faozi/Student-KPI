...
<body onLoad="show_AddEntry()">
    ...
    <section class="sec-2" id="sec-2">
        <h2 style="padding: 10px;" >List of activities</h2>
        ...
        <?php
            ...
            if(isset($_GET["id"]) && $_GET["id"] != ""){
                $id = $_GET["id"];
                $userID = $_SESSION["UID"];
                $sql = "SELECT * FROM activity WHERE act_id = ? AND userID = ?";
                $stmt = mysqli_prepare($conn, $sql);
        ...
        <div style="padding: 10px;" id="activityDiv">
            <h3 align="center">Edit activity</h3>
            ...
            <div id="inputForm">    
                <input type="hidden" id="aid" name="aid" value="<?=$_GET['id']?>">
                        ...
                        <label style="margin: 20px; display: inline;">Year*</label>
                            <?php
                            if($year!=""){
                                echo '<input type="text" name="year" size="5" pattern="\d{4}\/\d{4}" title="Enter a year range in the format \'YYYY/YYYY\'" value= "' . $year . '" required id="myInput">';
                            }
                            else {
                            ?>
                                <input type="text" name="year" size="5" pattern="\d{4}\/\d{4}" title="Enter a year range in the format 'YYYY/YYYY'" required id="myInput">
                            <?php
                            }
                            ?>                
                        <label>activity*</label>
                            <textarea rows="4" name="activity" cols="20" required id="myInput"><?php echo $activity;?></textarea>
                        <label>Remark</label>
                        ...
                        <label>Photo</label>
                            ...
                            <a href="#" id="remove" onclick="deleteImage('<?=$row["act_id"]?>', '<?=$row["img_path"]?>'); clearImagePath(); return false;">
...
function deleteImage(id, imgPath) {
    if (confirm('Delete?')) {
        ...
        // Send a GET request to delete_image.php with the activity ID and image path
        xhr.open('GET', 'delete_image_activity.php?id=' + id + '&img_path=' + imgPath, true);
        xhr.send();
    }
}
