...
<body onLoad="show_AddEntry()">
    ...
    <section class="sec-2" id="sec-2">
        ...
        <h2 style="padding: 10px;">List of KPI Competitions</h2>
        ...
        if (isset($_GET["id"]) && $_GET["id"] != "") {
            $id = $_GET["id"];
            $userID = $_SESSION["UID"];
            $sql = "SELECT * FROM kpicompetition WHERE id = ? AND userID = ?";
            $stmt = mysqli_prepare($conn, $sql);
        ...
        <div style="padding: 10px;" id="kpiDiv">
            <h3 align="center">Edit KPI Competition</h3>
            <p align="center" style="color: red;">Required fields are marked*</p>

            <form method="POST" action="competition_management_edit_action.php" enctype="multipart/form-data" id="myForm">
                <div id="inputForm">
                    <input type="hidden" id="kcid" name="kcid" value="<?= $_GET['id'] ?>">
                    ...
                    <select name="type" required>
                        <option value="1" <?php echo ($type == 1) ? 'selected' : ''; ?>>Faculty Competition</option>
                        <option value="2" <?php echo ($type == 2) ? 'selected' : ''; ?>>University Competition</option>
                        <option value="3" <?php echo ($type == 3) ? 'selected' : ''; ?>>National Competition</option>
                        <option value="4" <?php echo ($type == 4) ? 'selected' : ''; ?>>International Competition</option>
                    </select>
                    <label>Remark</label>
