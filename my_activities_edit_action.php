...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["fileToUpload"]) && $_FILES["fileToUpload"]["name"] == "") {
        $sql = "UPDATE activity SET sem = ?, year = ?, activity = ?, 
            remark = ? WHERE act_id = ? AND userID = ?";
        $stmt = mysqli_prepare($conn, $sql);

        // Bind parameters
        mysqli_stmt_bind_param($stmt, "issssi", $sem, $year, $activity, $remark, $id, $_SESSION["UID"]);
        ...
        if ($uploadOk) {
            ...
            $oldImagePathQuery = "SELECT img_path FROM activity WHERE act_id = ? AND userID = ?";
            ...
            if (!empty($oldImage)) {
                $oldImagePath = "activity/" . $oldImage;
            ...
            $sql = "UPDATE activity SET sem = ?, year = ?, activity = ?, 
            remark = ?, img_path = ? WHERE act_id = ? AND userID = ?";
            $stmt = mysqli_prepare($conn, $sql);

            // Bind parameters
            mysqli_stmt_bind_param($stmt, "issssii", $sem, $year, $activity, $remark, $uploadfileName, $id, $_SESSION["UID"]);
            ...
            if (mysqli_stmt_execute($stmt)) {
                if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
                    ...
                    header("Location: my_activities_edit.php");
                }
            } else {
                $_SESSION["error_message"] = "Error updating record: " . mysqli_error($conn);
                header("Location: my_activities_edit.php");
